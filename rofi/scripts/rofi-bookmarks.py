...
def firefox_in_current_workspace():
    current_workspace = subprocess.check_output(["hyprctl", "activeworkspace", "-j"])
    current_workspace = json.loads(current_workspace)["id"]

    windows = subprocess.check_output(["hyprctl", "clients", "-j"])
    windows = json.loads(windows)

    for win in windows:
        if win["workspace"]["id"] == current_workspace and "firefox" in win["class"].lower():
            return True
    return False
...
if __name__ == "__main__":
    ...
    if environ.get('ROFI_RETV') == '1':
        ...
        if firefox_in_current_workspace():
            subprocess.Popen(["firefox", "--new-tab", environ['ROFI_INFO']] + prof,
        ...
        print("\x00prompt\x1fbookmarks")  # change prompt
