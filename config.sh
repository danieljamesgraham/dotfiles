...
cron_job="0 4 * * * git -C $script_dir pull"
...
if [ -f "$HOME/.zshrc" ]; then
	if [ "$(readlink -f "$HOME/.zshrc")" == "$script_dir/.zshrc" ]; then
		echo ".zshrc already symlinked."
	else
		echo -e "\033[31m.zshrc already exists!\033[0m"
	fi
else
	ln -s "$script_dir/.zshrc" "$HOME/.zshrc" && echo ".zshrc symlink created."
fi
mkdir -p "$HOME/.zsh"
...
if [ -f "$HOME/.vimrc" ]; then
	if [ "$(readlink -f "$HOME/.vimrc")" == "$script_dir/.vimrc" ]; then
		echo ".vimrc already symlinked."
	else
		echo -e "\033[31m.vimrc already exists!\033[0m"
	fi
else
	ln -s "$script_dir/.vimrc" "$HOME/.vimrc" && echo ".vimrc symlink created."
fi
mkdir -p "$HOME/.vim"
...
if command -v nvim &> /dev/null; then
	...
	if [ -f "$HOME/.config/nvim/init.vim" ]; then
		if [ "$(readlink -f "$HOME/.config/nvim/init.vim")" == "$script_dir/.vimrc" ]; then
			echo "init.vim already symlinked."
		else
			echo -e "\033[31minit.vim already exists!\033[0m"
		fi
	else
		ln -s "$script_dir/.vimrc" "$HOME/.config/nvim/init.vim" && echo "init.vim symlink created."
	fi
...
if [ -f "$HOME/.tmux.conf" ]; then
	if [ "$(readlink -f "$HOME/.tmux.conf")" == "$script_dir/.tmux.conf" ]; then
		echo ".tmux.conf already symlinked."
	else
		echo -e "\033[31m.tmux.conf already exists!\033[0m"
	fi
else
	ln -s "$script_dir/.tmux.conf" "$HOME/.tmux.conf" && echo ".tmux.conf symlink created."
fi
mkdir -p "$HOME/.tmux"
